...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_tensor_flip = self.transform_flip(image) 
        id = self.list_id[index]
        name_image = os.path.basename(self.list_path[index])
        return id, name_image, image_tensor, image_tensor_flip
...
if __name__ == "__main__":
    ...
    features_flip_dir = "feature_flip_dir"
    ...
    model = ONNX_IMINT("/home/data2/tanminh/Evaluate_FIQA_EVR/pretrained/stacking_avg_r160+ada-unnorm-stacking-ada-1.6.onnx")
    # model.to("cuda")
    ...
    for _, (list_id, list_name, list_images, list_images_flip) in tqdm(enumerate(dataloader)):
        features = model(list_images.to("cuda"))
        features_flip = model(list_images_flip.to("cuda"))
        list_id = list(list_id)
        list_name = list(list_name)
        if isinstance(features, torch.Tensor):
            features = features.detach().cpu()

        for id, name, feature, feature_flip in zip(list_id, list_name, features, features_flip): 
            
            if id not in dict_name_features.keys():
                dict_name_features[id] = [name] 
            else: dict_name_features[id].append(name) 

            if curr_id is None: 
                curr_id = id 
            if curr_id == id: 
                ls_features.append(feature) 
                ls_features_flip.append(feature_flip)
            else: 
                if len(dict_name_features[curr_id]) > 3: 
                    np.save(os.path.join(features_dir, curr_id), ls_features)
                    np.save(os.path.join(features_flip_dir, curr_id), ls_features_flip)
                    
                else: 
                    del dict_name_features[curr_id]
                ls_features = [feature] 
                ls_features_flip = [feature_flip]
                curr_id = id 
